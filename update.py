...
class update:
    ...
    def update(self):
            conn = connect("127.0.0.1", "root", "", "employee")
            cr = conn.cursor()
            s="update employee_data set name='"+self.tf1.get()+"',age="+self.tf2.get()+",post='"+self.tf3.get()+"',salary="+self.tf4.get()+" where eid="+self.tf5.get();
            cr.execute(s)
            showinfo("","Table updated!!")
            conn.commit()
            conn.close()
    ...
    def __init__(self):
        ...
        self.bt1=Button(self.root,text="Update",command=self.update)
